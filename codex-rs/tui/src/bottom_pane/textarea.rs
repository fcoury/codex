...
impl TextArea {
    pub fn new() -> Self {
        Self::new_with_keymap(TextAreaKeymap::default_for_text())
    }

    pub fn new_with_keymap(keymap: TextAreaKeymap) -> Self {
        Self {
            text: String::new(),
            cursor_pos: 0,
    ...
    pub fn set_keymap(&mut self, keymap: TextAreaKeymap) {
        self.keymap = keymap;
    }
    ...
    pub fn input(&mut self, event: KeyEvent) {
        if self.keymap.newline.matches(event) {
            self.insert_str("\n");
            return;
        }
        if self.keymap.delete_backward_word.matches(event) {
            self.delete_backward_word();
            return;
        }
        if is_altgr(event.modifiers) {
            if let KeyCode::Char(c) = event.code {
                self.insert_str(&c.to_string());
            }
            return;
        }
        if self.keymap.backspace.matches(event) {
            self.delete_backward(1);
            return;
        }
        if self.keymap.delete_forward_word.matches(event) {
            self.delete_forward_word();
            return;
        }
        if self.keymap.delete_forward.matches(event) {
            self.delete_forward(1);
            return;
        }
        if self.keymap.kill_line_start.matches(event) {
            self.kill_to_beginning_of_line();
            return;
        }
        if self.keymap.kill_line_end.matches(event) {
            self.kill_to_end_of_line();
            return;
        }
        if self.keymap.yank.matches(event) {
            self.yank();
            return;
        }
        if self.keymap.word_left.matches(event) {
            self.set_cursor(self.beginning_of_previous_word());
            return;
        }
        if self.keymap.word_right.matches(event) {
            self.set_cursor(self.end_of_next_word());
            return;
        }
        if self.keymap.move_left.matches(event) {
            self.move_cursor_left();
            return;
        }
        if self.keymap.move_right.matches(event) {
            self.move_cursor_right();
            return;
        }
        if self.keymap.move_up.matches(event) {
            self.move_cursor_up();
            return;
        }
        if self.keymap.move_down.matches(event) {
            self.move_cursor_down();
            return;
        }
        if self.keymap.line_start.matches(event) {
            let move_up_at_bol = !matches!(event.code, KeyCode::Home);
            self.move_cursor_to_beginning_of_line(move_up_at_bol);
            return;
        }
        if self.keymap.line_end.matches(event) {
            let move_down_at_eol = !matches!(event.code, KeyCode::End);
