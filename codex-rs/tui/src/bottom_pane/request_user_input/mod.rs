...
impl RequestUserInputOverlay {
    ...
    fn handle_confirm_unanswered_key_event(&mut self, key_event: KeyEvent) {
        ...
        if self.keymap.rui_cancel.matches(key_event)
            || self.keymap.rui_option_clear.matches(key_event)
        {
            {
        ...
        if self.keymap.rui_option_up.matches(key_event) {
            state.move_up_wrap(2);
            return;
        }

        if self.keymap.rui_option_down.matches(key_event) {
            state.move_down_wrap(2);
            return;
        }

        if self.keymap.rui_submit_or_next.matches(key_event) {
        ...
        if self.keymap.list_pick_index.matches(key_event) {
...
impl BottomPaneView for RequestUserInputOverlay {
    ...
    fn handle_key_event(&mut self, key_event: KeyEvent) {
        ...
        if self.keymap.rui_prev_question.matches(key_event) {
            self.move_question(false);
            return;
        }

        if self.keymap.rui_next_question.matches(key_event) {
            self.move_question(true);
            return;
        }
        ...
        if self.has_options()
            ...
            && self.keymap.rui_prev_question.matches(key_event)
        {
            self.move_question(false);
            return;
        }
        ...
        if self.has_options()
            && matches!(self.focus, Focus::Options)
            && self.keymap.rui_next_question.matches(key_event)
        {
            self.move_question(true);
            return;
        }
        ...
        match self.focus {
            Focus::Options => {
                ...
                if self.keymap.rui_option_select.matches(key_event) {
                    self.select_current_option(true);
                    return;
                }

                if self.keymap.rui_option_clear.matches(key_event) {
                    self.clear_selection();
                    return;
                }

                if self.keymap.rui_option_to_notes.matches(key_event) {
            ...
            Focus::Notes => {
                ...
                if self.has_options() && self.keymap.rui_notes_to_options.matches(key_event) {
                    self.clear_notes_and_focus_options();
                    return;
                }
                if self.has_options()
                    && self.keymap.rui_notes_backspace_empty.matches(key_event)
                    && notes_empty
                {
                    self.save_current_draft();
                    if let Some(answer) = self.current_answer_mut() {
                ...
                if self.has_options()
                    && (self.keymap.rui_option_up.matches(key_event)
                        || self.keymap.rui_option_down.matches(key_event))
                {
                    let options_len = self.options_len();
                    if self.keymap.rui_option_up.matches(key_event) {
                        let moved = if let Some(answer) = self.current_answer_mut() {
                    ...
                    } else if self.keymap.rui_option_down.matches(key_event) {
                        let moved = if let Some(answer) = self.current_answer_mut() {
                            answer.options_state.move_down_wrap(options_len);
                            answer.answer_committed = false;
                            true
