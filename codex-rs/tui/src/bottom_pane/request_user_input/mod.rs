...
impl BottomPaneView for RequestUserInputOverlay {
    fn handle_key_event(&mut self, key_event: KeyEvent) {
        ...
        let allow_plain_nav = self.has_options() && matches!(self.focus, Focus::Options);
        if self.keymap.rui_prev_question.matches(key_event) && (!is_text_input || allow_plain_nav) {
            self.move_question(false);
            return;
        }
        if self.keymap.rui_next_question.matches(key_event) && (!is_text_input || allow_plain_nav) {
            self.move_question(true);
            return;
        }
        ...
        match self.focus {
            ...
            Focus::Notes => {
                ...
                if self.keymap.rui_submit_or_next.matches(key_event) {
                    self.ensure_selected_for_notes();
                    let result = self.composer.submit(false);
                    if !self.handle_composer_input_result(result) {
                        self.go_next_or_submit();
                    }
                    return;
                }
                if self.has_options() && !is_text_input {
                    let options_len = self.options_len();
                    if self.keymap.rui_option_up.matches(key_event) {
                        let moved = if let Some(answer) = self.current_answer_mut() {
                            answer.options_ui_state.move_up_wrap(options_len);
