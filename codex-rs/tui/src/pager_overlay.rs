...
impl TranscriptOverlay {
    ...
    ///
    /// This mirrors the splice performed on `App::transcript_cells` during
    /// `ConsolidateAgentMessage` so the overlay stays in sync with the main
    /// transcript.
    pub(crate) fn consolidate_cells(
        ...
        let follow_bottom = self.view.is_scrolled_to_bottom();
        // Clamp the range to the overlay's cell count to avoid panic if the
        // overlay has fewer cells than the main transcript (e.g. cells were
        // inserted after the overlay was opened).
