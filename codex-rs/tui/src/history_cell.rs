...
/// re-renders from it at any width. This replaces the run of
/// `AgentMessageCell`s after a stream finalizes, so tables and other
/// width-sensitive content reflow correctly on terminal resize.
#[derive(Debug)]
...
impl HistoryCell for AgentMarkdownCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        let mut lines: Vec<Line<'static>> = Vec::new();
        // Re-render markdown from source at the current width. Reserve 2
        // columns for the "• " / "  " prefix prepended below.
        ...
        // Use prefix_lines (not word_wrap_lines) so table rows with
        // box-drawing characters are not broken by word-wrapping. The
        // markdown renderer already constrains output to wrap_width.
...
|-----------|-------------|---------|------------------|
| Canary Rollout | 2026-01-15 | Completed | Canary remained at limited traffic longer than planned because p95 latency briefly regressed during cold-cache periods |
...
";
    ...
    fn consolidation_walker_replaces_agent_message_cells() {
        ...
        assert_eq!(start, 1, "should find all 3 agent cells starting at index 1");
        assert_eq!(end, 4);

        // Splice.
        let consolidated: Arc<dyn HistoryCell> =
            Arc::new(AgentMarkdownCell::new(source));
