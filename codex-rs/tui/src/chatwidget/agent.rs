...
async fn local_external_chatgpt_tokens(
    ...
    auth_manager
        ...
        .await
        .map_err(|err| format!("failed to refresh external ChatGPT auth: {err}"))?;
                    ...
                    InProcessServerEvent::ServerRequest(request) => {
                        ...
                        match request {
                            ...
                            ServerRequest::ChatgptAuthTokensRefresh { request_id, params } => {
                                match local_external_chatgpt_tokens(Arc::clone(&auth_manager)).await
                                {
                                    Err(reason) => {
                                        reject_server_request(
                                            &client,
...
mod tests {
    ...
    #[tokio::test]
    async fn local_external_chatgpt_refresh_uses_configured_external_refresher() {
        let codex_home = TempDir::new().expect("create temp dir");
        let config = ConfigBuilder::default()
            .codex_home(codex_home.path().to_path_buf())
            .build()
