...
#[tokio::test]
async fn chatwidget_agent_message_md_fence_table_renders_unicode() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    ...
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::InsertHistoryCell(cell) = app_ev {
            rendered_lines.extend(cell.display_lines(120));
        }
    }

    let rendered_text = rendered_lines
        .iter()
        .map(|line| {
            line.spans
                .iter()
    ...
    assert!(
        ...
        "expected unicode table rendering, got:\n{rendered_text}"
    );
}

#[tokio::test]
async fn chatwidget_agent_message_md_fence_multi_table_renders_unicode_at_narrow_width() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    ...
    chat.handle_codex_event(Event {
        id: "a1".into(),
        msg: EventMsg::AgentMessage(AgentMessageEvent {
            message: "Absolutely, here are several different Markdown table patterns you can paste into your renderer:\n\n```md\n### 1) Basic\n| Name  | Role     | Active |\n|-------|----------|--------|\n| Alice | Engineer | Yes    |\n| Bob   | Designer | No     |\n\n### 2) Alignment\n| Left Align | Center Align | Right Align |\n|:-----------|:------------:|------------:|\n| apple      | banana       |          10 |\n| cat        | dog          |         200 |\n```\n".to_string(),
        }),
    });
    ...
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::InsertHistoryCell(cell) = app_ev {
            rendered_lines.extend(cell.display_lines(49));
        }
    }
    ...
    assert!(
        rendered_text.contains('┌') && rendered_text.contains('└'),
        "expected unicode table rendering at narrow width, got:\n{rendered_text}"
    );
}
